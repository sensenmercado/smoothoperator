...
if (!isset($_GET['search'])) {
    ...
    $sql = "SELECT script_results.*, customers.*, users.username, users.id FROM script_results, customers, users where date(question_datetime) between ".sanitize($_POST['from_date'])." and ".sanitize($_POST['to_date'])." and script_results.job_id = ".sanitize($_POST['job'])." and script_results.customer_id = customers.id and script_results.user_id = users.id";
